...
private:
 ...
 #ifdef DIST
   ftype                    ddist, sddist, rhos, rhom1s, psolid, dadp, dadrho,
                            dad, dpdrho, dpdu, dp, rhold, pold, diste, Pe;
   bool                     disteq1;
   static ftype             uini;
...
public:
      double h, hMax;
      ErrorHTooBig(const double& _h) { h    = _h;
                                       hMax = Particle::hMax; }
   };
#endif
   ...
   void setWork(const double& _work)
   {
      ...
      ratio = Min(ratio, (double)2.);
      ratio = Max(ratio, double (0.5));
      work  = ratio * work;
      //}
   }
   ...
   void setWork(const float& _work)
   {
      ...
      ratio = Min(ratio, (float)2.);
      ratio = Max(ratio, float (0.5));
      work  = ratio * work;
      //}
   }
#endif
...
// equation of state
   void calleos()
   {
      ...
      rhos   = rho * dist;
      rhom1s = 1. / rhos;
      eos.eos(rhos, rhom1s, u,
              matNr(), dad, dist, psolid, vsound, T, dpdrho, dpdu, distmodel);
      p = psolid / dist;
 #else
   ...
   void plastic()
   {
      ...
      ftype sqJ2 = sqrt(
         .5 *
         (sxx * sxx + syy * syy + szz *
          szz) + sxy * sxy + sxz * sxz + syz * syz) + tiny;
...
//    sLim = 1.;
   ...
   void inverse()
   {
      ...
      detai =
         ...
         (double)(ai13 * ai22 *
                  ai31) -
         (double)(ai11 * ai23 * ai32) - (double)(ai12 * ai21 * ai33);
...
//    ftype rhoim2 = rhom1 * rhom1, rhojm2 = read->rhom1 * read->rhom1;
      ...
      if (matstr() && read->matstr() && (bodyNr() == read->bodyNr()))
      {
         ...
         ftype dvxyi = dv[0] * (d[0] * ci12 + d[1] * ci22 + d[2] * ci32),
               dvyxi = dv[1] * (d[0] * ci11 + d[1] * ci21 + d[2] * ci31);
         ftype dvxzi = dv[0] * (d[0] * ci13 + d[1] * ci23 + d[2] * ci33),
               dvzxi = dv[2] * (d[0] * ci11 + d[1] * ci21 + d[2] * ci31);
         ftype dvyzi = dv[1] * (d[0] * ci13 + d[1] * ci23 + d[2] * ci33),
               dvzyi = dv[2] * (d[0] * ci12 + d[1] * ci22 + d[2] * ci32);
         ...
         // Strain rate tensor
         write->epsxx -= gWmri * dv[0] *
                         (d[0] * read->ci11 + d[1] * read->ci21 + d[2] *
                          read->ci31);
         write->epsyy -= gWmri * dv[1] *
                         (d[0] * read->ci12 + d[1] * read->ci22 + d[2] *
                          read->ci32);
         write->epszz -= gWmri * dv[2] *
                         (d[0] * read->ci13 + d[1] * read->ci23 + d[2] *
                          read->ci33);

         ftype dvxyj = dv[0] *
                       (d[0] * read->ci12 + d[1] * read->ci22 + d[2] *
                        read->ci32), dvyxj = dv[1] *
                                             (d[0] * read->ci11 + d[1] *
                                              read->ci21 + d[2] *
                                              read->ci31);
         ftype dvxzj = dv[0] *
                       (d[0] * read->ci13 + d[1] * read->ci23 + d[2] *
                        read->ci33), dvzxj = dv[2] *
                                             (d[0] * read->ci11 + d[1] *
                                              read->ci21 + d[2] *
                                              read->ci31);
         ftype dvyzj = dv[1] *
                       (d[0] * read->ci13 + d[1] * read->ci23 + d[2] *
                        read->ci33), dvzyj = dv[2] *
                                             (d[0] * read->ci12 + d[1] *
                                              read->ci22 + d[2] *
                                              read->ci32);
      ...
      if (ti_eps > 0.)
      {
         if (sigxx < pij) sigxx = 0.;
         else sigxx -= pij;
         if (sigyy < pij) sigyy = 0.;
         else sigyy -= pij;
         if (sigzz < pij) sigzz = 0.;
         else sigzz -= pij;
         if (sigxy < 0.) sigxy = 0.;
         if (sigxz < 0.) sigxz = 0.;
         if (sigyz < 0.) sigyz = 0.;
   ...
   void forcesNewton(T* other)
   {
      ...
      if (bodyNr() == 0) gravH2 = gravHb12, gravHm1 = gravHb1m1, gravHm2 =
            gravHb1m2;
...
//    std::cout << "forcesQuad called" << std::endl;
   ...
   void distention()
   {
      ...
      ftype        cddm     =
         pow(pow(1 + deltadam, 0.333333) - pow(deltadam, 0.333333), -1);
      ...
      ftype  densitye = mt.rho0 *
                        (2.0 * mt.B - mt.A -
                         (mt.a +
                          mt.b) * uini * mt.rho0 +
                         pow((mt.A * mt.A + 4. * diste * mt.B * dmat.Pe +
                              (mt.a +
                               mt.b) * uini * mt.rho0 *
                              (-4 * mt.B + 2 * mt.A +
                               (mt.a + mt.b) * uini * mt.rho0)),
                             0.5)) / (2. * diste * mt.B);
      ftype  densitys = mt.rho0 *
                        (2.0 * mt.B - mt.A -
                         (mt.a +
                          mt.b) * uini * mt.rho0 +
                         pow((mt.A * mt.A + 4. * 1.0 * mt.B * dmat.Ps +
                              (mt.a +
                               mt.b) * uini * mt.rho0 *
                              (-4 * mt.B + 2 * mt.A +
                               (mt.a + mt.b) * uini * mt.rho0)),
...
//
      if (distmodel == 1)
      {
         if ((dmat.dist0 > 1.0) && (p < dmat.Ps))
         {
            ...
            {
               double aptmp1 = (double)pow(dmat.Pt - p, n1 - 1) * (double)pow(
                  dmat.Pt - dmat.Pe, -n1);
               dadps1 = -n1 * (diste - distt) * aptmp1;
               double aptmp2 = (double)pow(dmat.Ps - p, n2 - 1) * (double)pow(
                  dmat.Ps - dmat.Pe, -n2);
            ...
            {
               ddm = 0.3333333 * pow(
                  (-1) / (diste - 1) * (dist - 1) + 1 + deltadam,
                  -0.666666) * (-1) / (diste - 1) * cddm * ddist;
            }
            else
            {
               ddm = 0;
         ...
         ftype dadrhocf =
            ((p /
              (rho *
               rho) * dpdu + dist *
              dpdrho) / (dist + dadp * (-dpdrho * rho + p))) * dadp;
            ...
            if ((rho > densitye) && (rho > ref) || (diststate > 0.5))
            {
               ...
               ftype dadpr1  =
                  Min(-n1 *
                      (diste -
                       distt) *
                      pow(dmat.Pt - pr, n1 - 1) * pow(dmat.Pt - dmat.Pe,
                                                      -n1), 0.);
               ftype dadpr2  =
                  Min(-n2 *
                      (distt -
                       1.0) * pow(dmat.Ps - pr, n2 - 1) * pow(dmat.Ps - dmat.Pe,
                                                              -n2), 0.);
               ftype dadpr   = dadpr1 + dadpr2;
               ftype dprdrho = (mt.A + 2.0 * mt.B * ur) / mt.rho0;
               dadrhos =
                  (dist *
                   dprdrho) / (dist + dadpr * (-dprdrho * rho + pr)) * dadpr;
...
//
   ...
   void pstresses()
   {
      ftype Sxx  = reduce * sxx - p - grav;
      ftype Syy  = reduce * syy - p - grav;
      ftype Sxy  = reduce * sxy;
      ftype Sxz  = reduce * sxz;
      ftype Syz  = reduce * syz;
...
//       pa3  = Vector<ftype>(0.,0.,1.);
      ...
      else
      {
         ...
         ftype pp = -Sxx - Syy - Szz,
               q  = Sxx * Syy + Sxx * Szz + Syy * Szz - Sxy * Sxy - Sxz * Sxz -
                    Syz * Syz,
               r  = Sxx * Syz * Syz + Syy * Sxz * Sxz + Szz * Sxy * Sxy - Sxx *
                    Syy * Szz
                    - 2 * Sxy * Sxz * Syz,
               a  = third * (3. * q - pp * pp),
 ...
 #ifdef DIST
      // pressure cond
      if ((p > 0.8 * Pe) && (p < 1.2 * Pe) && (dist > 0.9 * diste) &&
          (diste > 0.)) if ((dp != 0.0) && (dp > 0.))
         {
...
ftype Particle::hMinm1, Particle::rhoMin, Particle::uMin, Particle::pMin,
Particle::dtpFactor;
...
ftype Particle::G;
ftype Particle::gravHb1, Particle::gravHb12, Particle::gravHb1m1,
Particle::gravHb1m2;
ftype Particle::gravHb2, Particle::gravHb22, Particle::gravHb2m1,
Particle::gravHb2m2;
