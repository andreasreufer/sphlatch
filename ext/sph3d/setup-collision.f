...
c
     ...
     2                syz(idim)
      common /work  / xt(idim), yt(idim), zt(idim), ht(idim),
     1                dmt(idim)
      common /rseed/ iseed
      common /oldc / ioldcode
c
      dimension ipind(idim), ipdam(idim), iprem(idim)
c
      character*70 title
...
c
  ...
  101 format(a15)
      read(30,*)iphys, ieos, ioldcode
      read(30,*)rt, nt, ftc, mattc, torutc, nfragtc, voidtc, 
     1          ftm, mattm, torutm, nfragtm, voidtm
      read(30,*)rp, np, fpc, matpc, torupc, nfragpc, voidpc, 
...
c  ===========
c
      call setparticles(iphys,ieos,rt,nt,ftc,mattc,torutc,ftm,mattm,
     1                  torutm,rcoret,nsett)
c
...
c
      if(nfragtc.gt.0)then
         ...
         enddo
         call fracture(np,nfragtc,rt,voidtc,ipdam,iprem)
         call cleanup(ntot,nsett,np,ipind,ipdam,iprem)
      endif
c
c  b) mantle
c
      if(nfragtm.gt.0)then
         ...
         enddo
         call fracture(np,nfragtm,rt,voidtm,ipdam,iprem)
         call cleanup(ntot,nsett,np,ipind,ipdam,iprem)
      endif
c
c--make projectile
c  ===============
c
      call setparticles(iphys,ieos,rp,np,fpc,matpc,torupc,fpm,matpm,
     1                  torupm,rcorep,nsetp)
c
...
c
      if(nfragtc.gt.0)then
         ...
         enddo
         call fracture(np,nfragpc,rp,voidpc,ipdam,iprem)
         call cleanup(ntot,nsetp,np,ipind,ipdam,iprem)
      endif
c
c  b) mantle
c
      if(nfragpm.gt.0)then
         ...
         enddo
         call fracture(np,nfragpm,rp,voidpm,ipdam,iprem)
         call cleanup(ntot,nsetp,np,ipind,ipdam,iprem)
      endif
c
c--assign flaws
c  ============
c
      call setflaws(nsett,rt,nsetp,rp)
c
c--setup collision
c  ===============
c
      call setcol(nsett,nsetp,rt,rp,angle,vrel,x1,y1,z1,x2,y2,z2)
c
...
c
      ...
      subroutine setparticles(iphys,ieos,rmax,napprox,fc,imatc,toruc,fm,
     1                        imatm,torum,rcore,nset)
c*****************************************************************
c                                                                *
...
c
      ...
      do while (id(istart).ne.0)
         istart=istart + 1
      enddo
      call setonlattice(istart,rmax,hsep,xmin,xmax,ymin,ymax,
     1                  zmin,zmax,nset)
c
c--set velocities
c
...
c
      ...
      subroutine inside(xi,yi,zi,rmax,iflg)
c************************************************************
c                                                           *
...
c
      ...
      subroutine setonlattice(istart,rmax,hsep,xmin,xmax,ymin,ymax,
     1                        zmin,zmax,nset)
c****************************************************************
c                                                               *
      ...
      do y0=ymin,ymax,ystep
         ...
         do x0=xmin,xmax,hsep
            do z0=zmin,zmax,zstep
               ...
               yi=y0
               zi=z0
               call inside(xi,yi,zi,rmax,iflg)
               if(iflg.eq.1)then
                  npart=npart + 1
                  nset=nset + 1
                  x(npart)=xi
...
c
      ...
      subroutine setcol(n1,n2,radt,radp,angle,vrel,x1,y1,z1,x2,y2,z2)
c********************************************************************
c                                                                   *
...
c
      ...
      subroutine setflaws(n1,rt,n2,rp)
c**************************************************************
c                                                             *
...
c
      ...
      call setf(iallow,ishield,i1,i2,rt)
c
c--second object
c  -------------
c
      ...
      call setf(iallow,ishield,i1,i2,rp)
c
c     close(41)
c
      return
      end
      subroutine setf(iallow,ishield,i1,i2,rad)
c**********************************************************
c                                                         *
...
c
     ...
     4                rho(idim)   , u(idim)
      common /fracm/ pweib(nmatmax), cweib(nmatmax)
      common /fract/ epsmin(idim), epsmax(idim), grav(idim),
     1               nflaws(idim), xm(idim),acoef(idim), young(idim)
     ...
     4               beta(nmatmax), capb(nmatmax)
      ...
      common /oldc / ioldcode
c
      double precision const, cweib, epsmean, eps, pw, vol,
     1                 xflaws, randm, rand2
c
c--initialize
c
      pi=acos(-1.)
...
c
      xflawsmax=1.0e7
      nflawsmax=int(xflawsmax)
      nfilled=0
      epsmean=0.d0
      xpart=nused
      do iflaw=1,nflawsmax
c
c  1) pick particle
c
         randm=rand2(iseed)
         i=int(randm*xpart) + i1
         imat=matter(i)
         pw=1.d0/pweib(imat)
...
c
         ...
         nflaws(i)=nflaws(i)+1
      enddo
      epsmean=epsmean/xflawsmax
c
c--checking for holes
c
      ntofill=i2-i1 + 1
      nholes=0
      if(nfilled.lt.ntofill)then
         do i=i1,i2
            if(epsmin(i).eq.0.)then
               epsmin(i)=const*xflawsmax**pw
               nflaws(i)=1
               nholes=nholes + 1
            endif
         enddo
      end if
      write(*,*)'nb of holes plugged: ',nholes
c
c--compute the local Weibull distribution
c
      write(*,*)'---> writing flaws in file: ',i1,i2
c
c--new code
c
      if(ioldcode.eq.0)then
         do i=i1,i2
            if(nflaws(i).eq.1) then
               xm(i)=1.
            else 
               xm(i)=log(float(nflaws(i)))/log(epsmax(i)/epsmin(i))
            endif
...
c
  ...
  100       format(2(1pe13.6,1x),i6,2x,3(1pe13.6,1x))
         enddo
      endif
c
      return
...
c
      ...
      subroutine fracture(np,nseed,rad,voidf,ipdam,iprem)
c**************************************************************
c                                                             *
...
c
      dimension iflag(idim), neilist(idim), listseed(nbseed) 
      dimension listdam(idim), ipdam(idim), iprem(idim)
c
...
c
    ...
    5    call randombody(np,ipart)
         ...
         r=sqrt(xt(ipart)**2+yt(ipart)**2+zt(ipart)**2)
         frac=(r/rad)**expdist
         random=rand2(iseed)
...
c
      ...
      subroutine mllist (npart,x,y,z,h)
c************************************************************
c                                                           *
...
c
      ...
      subroutine cleanup(npart,npset,np,ipind,ipdam,iprem)
c***************************************************************************
c                                                                          *
...
c
     ...
     2                syz(idim)
c
      dimension ipind(idim), ipdam(idim), iprem(idim), iremove(idim)
c
      do i=1,idim
         iremove(i)=0
      enddo
...
c
      character*70 title
      character*15 filename
      character*28 varnam(maxvars), varnam2(maxvars)
c
