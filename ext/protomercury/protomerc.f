      Program interior 
c***************************************************************************
c For the interior structure of the planets, accomplished by Michael Fuchs *
c*************************************************************************** 
      implicit double precision (a-h,o-z)
      integer kmax,kount,nok,nvar,nbad,maxmat
      parameter (nvar=3)
      parameter(maxmat=21)
      parameter(MAXLOOP=500)
      integer j,n
...
c------------------------------------------------------------
c                                                           !
c      'totrad [cm],total mass [g]                          !
c                                                           !
c      'mass of core/total mass [0-1]:mc_vs_mtot '          !                   
c                                                           !
c      'Upper limit for mean density [g/cm^3]:'             !
c       RhoUpperLimit                                       !
c                                                           !  
c      'Lower limit for mean density [g/cm^3]:'             !
c       RhoLowerLimit                                       !
c                                                           !
c      'Surface temperature [K]: tsurf'                     !
c                                                           !
c      'Surface density [g/cm^3]: rhosurf'                  !
c                                                           !
c-----------------------------------------------------------!
      open(29,file='initial-values')
      read(29,*) totrad,totmass,mc_vs_mtot
      write(*,*) totrad,totmass,mc_vs_mtot
      read(29,*) RhoUpperLimit,RhoLowerLimit
      write(*,*) RhoUpperLimit,RhoLowerLimit
      read(29,*) tsurf,rhosurf
      write(*,*) tsurf,rhosurf
      pause
c
...
c
      ...
      itdone    = 0        ! itdone indicates radius iteration status 
                           ! (0:running,1:done)
c
      do 1203 loopcount = 1,MAXLOOP   ! Do at most MAXLOOP loops 
                                      ! (RADIUS ITERATION)
c
          errid     = 0      ! Error index (concerning rad.iteration)
          small     = 1.d-5  ! Desired upper limit for radius
                             ! (of center to last int.point)/totrad
          kmax      = 200    ! Maximum number of values to be stored
          x1        = 0.d0   ! Integrate from planets surface -
          x2        = 1.d0-(mc_vs_mtot)**(2.d0/3.d0)  ! - to core boundary. 
                             ! Int.variable is mu=1-(m/mtot)^(2/3)
          dxsav     = (x2-x1)/199.d0                  
...
c
          if (loopcount.eq.1) then              
             call aneos2(1,nmat,0,nntype) ! Initialize material just 
                                          ! in the first loop of rad.integration
          endif
c     
          call odeint(ystart,nvar,x1,x2,eps,h1,hmin,nok,nbad,  
                                          ! Start integration of mantle
     &         derivs,rkqs)                     
c          
          write(*,*) nok,nbad   ! How many accepted/rejected (by rkqs) steps ?
c
          if (itdone.eq.1) then                    ! If rad.iteration is done, 
...
c     
          ...
          close(22)
c
          write(*,*)'Radii ratio:',(yp(2,kount)/yp(2,1))**(1.d0/2.d0)  
                                ! Ratio of r(core)/totrad
c     
          x1        = 1.d0-(mc_vs_mtot)**(2.d0/3.d0)     
                                ! Now integrate from core boundary - to center
          x2        = 1.d0                             
          dxsav     = (x2-x1)/199.d0
          eps       = 1.d-16                             
          h1        = 1.d-12                             
          hmin      = 0.0d0                              
          ystart(1) = yp(1,kount)  ! starting funct.value: 
c                                    last funct.value of mantle integration
          ystart(2) = yp(2,kount)               
c                                   ystart(3): density is found by iteration 
c                                   (see below)     
          klst      = 24
          kinp      = 23
          imat      = 5                                ! 5:iron
...
c            Initialize material just in first loop of rad.integration
          endif   
c-------------------------------------------------------------------------------     
...
c
           if (itdone.eq.1) then
              open(20,file='protomercury', status='old')
...
c      write(*,*) 'LAMBDA,ZETA,ETA,XI',plambda,zeta,eta,xi
      dydx(1) = alpha/(cp*dexp(plambda))*3.d0*gg*totmass**2.d0/
...
c
             ...
             write(*,*) itdone
             open(19,file='finalradius')
             write(19,*) 'Final radius:  ',totrad
             write(*,*)
             pause
             return
          endif
