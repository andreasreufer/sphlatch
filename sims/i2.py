...
vimpimpaa = [ \
 (25., 1.8), (30., 1.8), (35., 1.8),
 (25., 2.0), (30., 2.0), (35., 2.0),
 (25., 2.5), (30., 2.5), (35., 2.5),
 (45., 1.05), (60., 1.05), 
 (45., 1.10), (60., 1.10), 
 (45., 1.15), (60., 1.15) ]
...
def finalizeSims(sims=simadm.getSimsByState("finished")):
  archsims = []
  for sim in sims:
    sim._getResults()
    if sim.results.valid:
  ...
  simadm.archiveSims(archname, archsims)
...
def newmsssims():
  for line in file("msssim.txt").readlines():
    skey = line.replace("\n","")

    if not sims.has_key(skey):
       sim = simadm.newSim(SimParam( skey ) )
       sim._setCourant(0.1)
       sim.next()

def finalizePartSims(trelfin = 15.):
  finsim = simadm.getSimsByState("finished")
  
  for sim in simadm.getSimsByState("partial"):
    if ( sim.tlast / sim.tcol ) > trelfin:
      finsim.append(sim)
