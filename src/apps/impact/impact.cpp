...
valueType timeStep()
{
  ...
  ///
  const valueType minCoreDensity = PartManager.attributes["mincoredens"];
  valueType comX = 0., comY = 0., comZ = 0., comM = 0.;
  for (size_t i = 0; i < noParts; i++)
    {
  ...
  for (size_t i = 0; i < noParts; i++)
    {
      const valueType curRad = sqrt((pos(i, X) - comX) * (pos(i, X) - comX) +
  ...
  for (size_t i = 0; i < noMassBins; i++)
    {
      massRad(i) = maxRad * (i / static_cast<valueType>(noMassBins));
      massBins(i) = 0.;
    }
  ...
  for (size_t i = 0; i < noParts; i++)
    {
      ...
      ///
      const valueType rx = (pos(i, X) - comX);
      const valueType ry = (pos(i, Y) - comY);
      const valueType rz = (pos(i, Z) - comZ);

      const valueType r = sqrt(rx * rx + ry * ry + rz * rz);

      const valueType vx = vel(i, X);
      const valueType vy = vel(i, Y);
      const valueType vz = vel(i, Z);
