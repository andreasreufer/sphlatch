...
value_type xmin, xmax, ymin, ymax, zmin, zmax;
value_type sidelength;
...
value_type getSidelength();
vector_type getCenter();
size_t getDepth();
...
bool CostZone<Indices, SimTrait>::CreateParticleBucket(bucket_reference Bucket = particle_bucket)
{
  ...
  xmin = communication_manager.template Min<X>();
  xmax = communication_manager.template Max<X>();
  ymin = communication_manager.template Min<Y>();
  ymax = communication_manager.template Max<Y>();
  zmin = communication_manager.template Min<Z>();
  zmax = communication_manager.template Max<Z>();

  sidelength = ( xmax - xmin );  
  sidelength = ( ymax - ymin ) > sidelength ? ( ymax - ymin ) : sidelength;
  sidelength = ( zmax - zmin ) > sidelength ? ( zmax - zmin ) : sidelength;
  
  const value_type dist_per_index = sidelength
                                    / static_cast<value_type>(side-1);
  
  const value_type xoffset = ( xmin + xmax - sidelength ) / 2.; 
  const value_type yoffset = ( ymin + ymax - sidelength ) / 2.; 
  const value_type zoffset = ( zmin + zmax - sidelength ) / 2.; 

  size_t i,j,k;
  for (size_t cp = 0; cp < DSIZE; ++cp)
    {
      matrix_row Current(Data, cp);
      /*i = static_cast<size_t>( (Current(X) - xoffset) / dist_per_index );
      j = static_cast<size_t>( (Current(Y) - yoffset) / dist_per_index );
      k = static_cast<size_t>( (Current(Z) - zoffset) / dist_per_index );*/
      i = lrint( (Current(X) - xoffset) / dist_per_index );
      j = lrint( (Current(Y) - yoffset) / dist_per_index );
      k = lrint( (Current(Z) - zoffset) / dist_per_index );
      //std::cout << i << " " << j << " " << k << "\n";
...
bool CostZone<Indices, SimTrait>::CreateDomainMap(domain_map_reference map = domain_map)
{
  ...
  for (size_t i = 0; i < SIZE; i++)
    {
      /** OOSPH_LOADBALANCE enables loadbalancing */
#ifdef OOSPH_LOADBALANCE
...
template <typename Indices, typename SimTrait>
typename CostZone<Indices, SimTrait>::vector_type
CostZone<Indices, SimTrait>::getCenter(void)
{
  vector_type retvect(3);
...
template <typename Indices, typename SimTrait>
typename CostZone<Indices, SimTrait>::value_type
CostZone<Indices, SimTrait>::getSidelength(void)
{
  return sidelength;
}

template <typename Indices, typename SimTrait>
size_t CostZone<Indices, SimTrait>::getDepth(void)
{
  return depth;
}
