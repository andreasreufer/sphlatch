#ifndef OOSPH_IO_MANAGER_H
#define OOSPH_IO_MANAGER_H
...
public:
    ...
    typedef typename SimTrait::string_reference string_reference;
...
std::vector<int> IOManager<SimTrait>::LoadCDAT(std::string FileName)
{
	...
	for ( int i = 0; i < numparams; i++ ) {
		...
		if ( paramlabel == "FLOAT" ) { datawidthLoad = sizeOfXDRfloat ;}	
		else if ( paramlabel == "DOUBLE" ) { datawidthLoad = sizeOfXDRdouble ;}	
		else if ( paramlabel == "XDRD" ) {}	
		else if ( paramlabel == "DIMENSION" ) {}	
		else { MemoryManager.SaveParameter(paramlabel, lexical_cast<value_type>(paramvalue), true);}
		};
    ...
    for ( int i = 0; i < numattributes; i++ ) {
		...
		if (attributes[i] > attrmaxindex) { attrmaxindex = attributes[i]; }
	}
    ...
    if (datawidthLoad == sizeOfXDRfloat) {
		...
		for (int j = 0; j < numattributes; j++) {
			...
			if (attributes[j] != -1) { Data(i, attributes[j]) = (value_type)*valuep; }
			}
		}
...
bool IOManager<SimTrait>::SaveCDAT(std::string FileName, std::vector<int> Attributes)
{
    ...
    if (RANK == 0) { dataoffset = 0; } else { dataoffset = numremainparticles[RANK-1] * datawidthSave * numattributes; }
